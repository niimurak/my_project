    ...
    if ((Pdevl[0] == "") or (miny == 0 and maxy == 0)):
        ...
        for j in range(5):
            if Pdevl[j] != '':
                if j == 0:
                    ind = 0    # カーソル位置のインデックス
                    cur_v = Ax.axvline(xdate[dno][ind], color='k', linestyle='--', linewidth=0.5)
                    cur_h = Ax.axhline(yvalue[dno][ind], color='k', linestyle='--', linewidth=0.5)                
                    cur_point, = Ax.plot(xdate[dno][ind], yvalue[dno][ind], color='k', markersize=10, marker='o')
                    Ax.set_title('index: {}, x = {}, y = {}'.format(
                                ind, xdate[dno][ind], round(yvalue[dno][ind], 4)))
                    Ax.plot(xdate[j],yvalue[j],label=Pdevl[j] + ":" + Pnamel[j],marker='o',picker=15)
                else:                   
                    Ax.plot(xdate[j],yvalue[j],label=Pdevl[j] + ":" + Pnamel[j],marker='o',picker=15)
...
# キーボードイベント（座標表示位置移動）
def on_key(event):
    global Pdevl, Fig, Ax, dno, ind, xdate, yvalue, cur_v, cur_h, cur_point
    ...
    # インデックスの更新
    ind += move
    ind %= len(xdate[0])  # グラフの端に行くと戻るようにする
    dno += ud
    dno %= dnmx  # グラフの端に行くと戻るようにする
    # カーソルとタイトルの更新 Line2D
    cur_v.set_xdata(xdate[dno][ind])
    cur_h.set_ydata(yvalue[dno][ind])
    cur_point.set_data(xdate[dno][ind], yvalue[dno][ind])
    Ax.set_title('index: {}, x = {}, y = {}'.format(
                                    ind, xdate[dno][ind], round(yvalue[dno][ind], 4)))
    Fig.canvas.draw_idle()

# マウスイベント(クリック時にカーソル位置の座標を表示）
def on_click(event):
    global Fig, Ax

    if (event.xdata is  None) or (event.ydata is  None):
        return
    xd = datetime.datetime.utcfromtimestamp(int(event.xdata*24*3600))
    Ax.set_title('x = {}, y = {}'.format(xd, round(event.ydata, 3)))
    Fig.canvas.draw_idle()
